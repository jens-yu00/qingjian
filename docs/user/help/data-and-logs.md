...
本 fork 提供一次性离线迁移工具，目前只核验过 macOS 微信输入法 1.4.3。它把个人词频、精确全拼下的选词偏好和相关用户词合并到青简现有学习数据中；普通「导入词库」菜单不执行这项迁移。

迁移后，同一拼音下常选的词会获得个人排序权重，严格全拼设置继续生效。上下文、简拼、补全和纠错偏好不迁移，两款输入法的完整候选顺序可能不同。

导入前需切换输入源、让青简保存学习数据并停止运行，完成后重新启用。迁移原件与备份留在本机；此次个人安装的备份位于上述数据目录的 `migrations/wetype-20260915-134419/prepared-final/backup/`，相邻的 `plan.json` 记录原文件状态。数据目录中的 `wetype-migration-receipt.json` 防止重复导入。恢复备份会放弃迁移后积累的相关学习记录，恢复前应另存当前文件。
